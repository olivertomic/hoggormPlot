        ...
        if item == 6:
            ...
            if individual:
                ...
                if which[plotInd] == 'X':
                    ...
                    varNames = YvarNames
                if cumulative == False:
                    CalExplVar_indVar = np.hstack(
                        [np.reshape(CalExplVar_indVar[:, 0], [-1, 1]), np.diff(CalExplVar_indVar)])
                ...
                if validated == False:
                    ...
                    try:
                        ...
                        if cumulative == False:
                            ValExplVar_indVar = np.hstack(
                                [np.reshape(ValExplVar_indVar[:, 0], [-1, 1]), np.diff(ValExplVar_indVar)])
